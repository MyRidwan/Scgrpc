...
BURIQ () {
    curl -sS  https://raw.githubusercontent.com/myridwan/myridwan.github.io/ipuk/izin > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/myridwan/myridwan.github.io/ipuk/izin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/myridwan/myridwan.github.io/ipuk/izin | awk '{print $4}' | grep $MYIP )
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
